...
public class AuthRegisterCommandValidator : AbstractValidator<AuthRegisterCommand>
{
    ...
    private async Task<bool> BeUniqueUserAsync(AuthRegisterCommand model, CancellationToken cancellationToken)
    {
        return !await _identityService.CheckEmailExistsAsync(model.Email, cancellationToken);
    }
}